...
type unionField struct {
	...
	common   *deserializeField
	variants []unionFieldVariant
}

type unionRequiredMarshalKind int

const (
	unionRequiredSliceField unionRequiredMarshalKind = iota
	unionRequiredArrayUnionField
	unionRequiredValueField
)

type unionRequiredMarshalField struct {
	discriminatorValue string
	field              unionField
	common             bool
	kind               unionRequiredMarshalKind
}
...
func discriminatorUnionCode(defs map[string]*jsonschema.Schema, name string, schema *jsonschema.Schema) ([]jen.Code, bool) {
	...
	needsMeta := false
	addField := func(jsonName string, prop *jsonschema.Schema, common bool, discriminatorValue string) {
		...
		if common {
			...
			field.variants = append(field.variants, unionFieldVariant{discriminatorValue: discriminatorValue, field: source})
		}
		fieldsByJSON[jsonName] = field
		if !contains(fieldOrder, jsonName) {
	...
	for _, branch := range branches {
		...
		for _, jsonName := range sortedPropertyNames(properties) {
			...
			if contains(branchRequired, jsonName) && prop.Const == nil && jsonName != discriminator {
				typ, text := schemaType(defs, prop, false)
				field := unionField{deserializeField: newDeserializeField(defs, jsonName, prop, typ, text), schema: prop}
				switch {
				...
				case discriminator != "":
					requiredMarshalFields = append(requiredMarshalFields, unionRequiredMarshalField{discriminatorValue: value, field: field, kind: unionRequiredValueField})
				}
			}
			addField(jsonName, prop, false, value)
		}
	}
	for _, req := range schema.Required {
		...
		if strings.HasPrefix(text, "[]") {
			...
			requiredMarshalFields = append(requiredMarshalFields, unionRequiredMarshalField{field: unionField{deserializeField: deserialize, schema: prop}, common: true, kind: unionRequiredSliceField})
		}
	}
	...
	for _, jsonName := range fieldOrder {
		...
		if field.common != nil {
			field.common.goName = field.goName
		}
		for i := range field.variants {
			field.variants[i].field.goName = field.goName
		}
		fieldsByJSON[jsonName] = field
	}
	...
	for i := range requiredMarshalFields {
		if field, ok := fieldsByJSON[requiredMarshalFields[i].field.jsonName]; ok {
			requiredMarshalFields[i].field.goName = field.goName
			requiredMarshalFields[i].field.typeCode = field.typeCode
			requiredMarshalFields[i].field.typeText = field.typeText
		}
		if requiredMarshalFields[i].kind == unionRequiredValueField && isRequiredInAllVariants(parentRequired, requiredCount, requiredMarshalFields[i].field.jsonName, len(branches)) {
			continue
		}
		filteredRequiredMarshalFields = append(filteredRequiredMarshalFields, requiredMarshalFields[i])
	}
	requiredMarshalFields = filteredRequiredMarshalFields
	...
	customUnmarshal := map[string]jen.Code{}
	for _, jsonName := range fieldOrder {
		field := fieldsByJSON[jsonName]
		deserializeFields = append(deserializeFields, field.deserializeField)
		if discriminator != "" && unionFieldNeedsVariantUnmarshal(field) {
...
func mergeUnionField(defs map[string]*jsonschema.Schema, existing, field unionField) unionField {
	...
	kind := schemaKind(defs, field.schema)
	if kind != schemaKind(defs, existing.schema) || !isPrimitiveConstKind(kind) {
...
func unionFieldNeedsVariantUnmarshal(field unionField) bool {
	...
	var sources []deserializeField
	if field.common != nil {
		sources = append(sources, *field.common)
	}
	for _, variant := range field.variants {
		sources = append(sources, variant.field)
	}
	if len(sources) < 2 {
		return false
	}
	for _, source := range sources[1:] {
		if !sameDeserializeField(sources[0], source) {
			return true
		}
	}
	return false
...
func sameDeserializeField(a, b deserializeField) bool {
	...
	return a.itemValidator.discriminator == b.itemValidator.discriminator && slices.Equal(a.itemValidator.values, b.itemValidator.values)
}

func discriminatorUnionFieldUnmarshalCode(defs map[string]*jsonschema.Schema, field unionField, discriminator, discriminatorGoName string, branches []*jsonschema.Schema, constNames map[string]string) jen.Code {
	var cases []jen.Code
	var fallback []jen.Code
	...
	for _, branch := range branches {
		...
		seen[value] = true
		source, ok := unionFieldSource(field, value)
		if !ok {
			continue
		}
		...
		caseValue := jen.Lit(value)
		if constName := constNames[value]; constName != "" {
			caseValue = jen.Id(constName)
		}
		cases = append(cases, jen.Case(caseValue).Block(body...))
	}
	if len(fallback) > 0 {
		cases = append(cases, jen.Default().Block(fallback...))
	}
	return jen.If(jen.Len(jen.Id("raw").Dot(field.goName)).Op(">").Lit(0)).Block(
		jen.Switch(jen.Id("decoded").Dot(discriminatorGoName)).Block(cases...),
	)
}

func unionFieldSource(field unionField, discriminatorValue string) (deserializeField, bool) {
	for _, variant := range field.variants {
		if variant.discriminatorValue == discriminatorValue {
			return variant.field, true
		}
	}
	if field.common != nil {
		return *field.common, true
	}
	return deserializeField{}, false
}
...
func discriminatorUnionMarshalCode(defs map[string]*jsonschema.Schema, name, discriminator string, requiredFields []unionRequiredMarshalField, constNames map[string]string) jen.Code {
	...
	for _, required := range requiredFields {
		...
		commonAssignments = append(commonAssignments, marshalFieldAssignmentCode(defs, receiver, required)...)
	}

	type discriminatorGroup struct {
		value  string
		fields []unionRequiredMarshalField
		seen   map[string]bool
	}
	...
	for _, required := range requiredFields {
		...
		group.fields = append(group.fields, required)
	}

	var cases []jen.Code
	for _, group := range groups {
		var assignments []jen.Code
		for _, required := range group.fields {
			assignments = append(assignments, marshalFieldAssignmentCode(defs, receiver, required)...)
		}
...
func discriminatorlessUnionMarshalCode(defs map[string]*jsonschema.Schema, name string, requiredFields []unionRequiredMarshalField, branchCount int) jen.Code {
	receiver := receiverName(name)
	fieldSeen := map[string]bool{}
	...
	sort.Slice(orderedFields, func(i, j int) bool {
		return orderedFields[i].field.jsonName < orderedFields[j].field.jsonName
	})
	...
	for _, required := range orderedFields {
		...
		if commonFields[field.goName] || fieldCounts[field.goName] == branchCount {
			body = append(body, marshalFieldAssignmentCode(defs, receiver, required)...)
			continue
		}
...
func optionalWireFieldAssignmentCode(receiver string, field unionField) []jen.Code {
	...
	default:
		condition = jen.Op("!").Qual("reflect", "ValueOf").Call(jen.Id(receiver).Dot(field.goName)).Dot("IsZero").Call()
	}
	return []jen.Code{
...
func marshalFieldAssignmentCode(defs map[string]*jsonschema.Schema, receiver string, required unionRequiredMarshalField) []jen.Code {
	switch required.kind {
	case unionRequiredSliceField:
		return sliceFieldAssignmentCode(receiver, required.field)
	case unionRequiredArrayUnionField:
		return arrayUnionFieldAssignmentCode(defs, receiver, required.field)
	default:
		return valueFieldAssignmentCode(receiver, required.field)
	}
}

func valueFieldAssignmentCode(receiver string, field unionField) []jen.Code {
	return []jen.Code{
		jen.Id(field.goName).Op(":=").Id(receiver).Dot(field.goName),
		jen.Id("w").Dot(field.goName).Op("=").Op("&").Id(field.goName),
	}
...
func arrayUnionFieldAssignmentCode(defs map[string]*jsonschema.Schema, receiver string, field unionField) []jen.Code {
	arrayUnion := constructorArrayUnionSchema(defs, field.schema)
	if arrayUnion == nil {
		return valueFieldAssignmentCode(receiver, field)
	}
	variants := arrayUnionVariants(defs, field.typeText, unionBranches(arrayUnion))
	if len(variants) == 0 {
		return valueFieldAssignmentCode(receiver, field)
	}
	flatVariant, _ := arrayUnionDecodeVariants(defs, variants)
	condition := jen.Id(field.goName).Dot(variants[0].fieldName).Op("==").Nil()
	for _, variant := range variants[1:] {
		condition.Op("&&").Id(field.goName).Dot(variant.fieldName).Op("==").Nil()
	}
	return []jen.Code{
		jen.Id(field.goName).Op(":=").Id(receiver).Dot(field.goName),
		jen.If(condition).Block(
			jen.Id("flat").Op(":=").Id(flatVariant.typeName).Values(),
			jen.Id(field.goName).Op("=").Id(field.typeText).Values(jen.Id(flatVariant.fieldName).Op(":").Op("&").Id("flat")),
		),
		jen.Id("w").Dot(field.goName).Op("=").Op("&").Id(field.goName),
	}
}
...
func unionConstructorCode(defs map[string]*jsonschema.Schema, unionName, discriminator string, branch *jsonschema.Schema, constructorName string, inline bool, parentProperties map[string]*jsonschema.Schema, parentRequired []string, fieldsByJSON map[string]unionField, constNames map[string]string) jen.Code {
	...
	for _, req := range paramJSONNames {
		prop := properties[req]
		field := newUnionField(defs, req, prop)
	...
	for _, jsonName := range sortedPropertyNames(properties) {
		...
		if constName := constFields[jsonName]; constName != "" {
			source := newUnionField(defs, jsonName, prop)
			literalFields = append(literalFields, jen.Id(field.goName).Op(":").Add(convertUnionFieldValue(field.typeText, source.typeText, jen.Id(constName))))
			continue
		}
...
func schemaKind(defs map[string]*jsonschema.Schema, schema *jsonschema.Schema) string {
	...
	branches := unionBranches(schema)
	if len(branches) > 0 {
		kind := schemaKind(defs, branches[0])
		for _, branch := range branches[1:] {
			if schemaKind(defs, branch) != kind {
				return ""
			}
		}
		return kind
...
func assignUnionField(fieldType, paramType, paramName string) jen.Code {
	if target, ok := strings.CutPrefix(fieldType, "*"); ok && target == paramType {
		return jen.Op("&").Id(paramName)
	}
	return convertUnionFieldValue(fieldType, paramType, jen.Id(paramName))
}

func convertUnionFieldValue(fieldType, valueType string, value jen.Code) jen.Code {
	if fieldType == valueType || fieldType == "" || fieldType == "any" || valueType == "" {
		return value
	}
	if target, targetPointer := strings.CutPrefix(fieldType, "*"); targetPointer {
		source, sourcePointer := strings.CutPrefix(valueType, "*")
		parameterType := jen.Id(valueType)
		body := []jen.Code{
			jen.Id("converted").Op(":=").Id(target).Call(jen.Id("v")),
			jen.Return(jen.Op("&").Id("converted")),
		}
		if sourcePointer {
			parameterType = jen.Op("*").Id(source)
			body = []jen.Code{
				jen.If(jen.Id("v").Op("==").Nil()).Block(jen.Return(jen.Nil())),
				jen.Id("converted").Op(":=").Id(target).Call(jen.Op("*").Id("v")),
				jen.Return(jen.Op("&").Id("converted")),
			}
		}
		return jen.Func().Params(jen.Id("v").Add(parameterType)).Op("*").Id(target).Block(body...).Call(value)
	}
	if strings.HasPrefix(valueType, "*") {
		panic(fmt.Sprintf("cannot assign nullable union field type %s to %s", valueType, fieldType))
	}
...
func arrayUnionMarshalCode(name string, variants []arrayUnionVariant) jen.Code {
	var body []jen.Code
	for _, variant := range slices.Backward(variants) {
